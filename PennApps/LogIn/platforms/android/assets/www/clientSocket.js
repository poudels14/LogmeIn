...
var Socket_connection = {
    ...
    "onConnected" : function(){
        ...
        var sM = "socket.on('pushInf', function(data){Socket_connection.parsePushInf(data)})";
    ...
    "sendLogin" : {
        ...
        "showDifferentLogins" : function(socketId, IP, domain){
            ...
            if(html == "") {
                ...
                window.alert("No logins found! This might be your first time, so sync your logins.");
            }
            else {
                html = x1 + html + x2;
    ...
    "parsePushInf" : function(data){
        ...
        if(json.title == "sync") {
            Socket_connection.syncLogin(data);
        }
    ...
    "syncLogin" : function(data) {

        alert("login received");
        var json_data = JSON.parse(data);
        var domain = json_data.data.domain;

        var db = getDatabase();
        var db_json = JSON.parse(db);

        var getSiteName = function(x){
            var it = x.split(".");
            if(it[it.length - 2]){
        ...
        for(var i = 0; i < db_json.length; i++) {
            if(matchDomain(db_json[i].domain, json_data.data.domain)) {

                for(var j = 0; j < db_json[i].logins.length; j++) {
                    if(db_json[i].logins[j].title == getTitle(json_data.data.username)) {
                        var site = getSiteName(domain);
                        var domain = json_data.data.domain;
                        var newUsername = json_data.data.username;
                        var newPassword = json_data.data.password;
                        ...
                        if(checkDatabase(site, domain)) {
                            ...
                            if(loginPushed && addToDatabase(JSON.stringify(db_json))) {
                                alert("Logins Updated.");
                            }
                            else {
                                console.log("Logins not changed.");
                            }
                        }
                        return;
                    }
                ...
                addNewLogin.addNewLogin(getSiteName(domain).capitalize(), getTitle(json_data.data.username), domain, json_data.data.username, json_data.data.password);
                return;
            }
        }

        addNewSite.commit(getSiteName(domain).capitalize(), domain, getTitle(json_data.data.username), json_data.data.username, json_data.data.password);

        // var checkDomain = function(x, y){
        //     var it = y.split(".");
        //     if(x.match(it[it.length - 2])){
        //         return true;
        //     }
        ...
        // if(checkDomain) {
        //     addNewSite.commit(json_data.data.domain, json_data.data.domain, "Title", json_data.data.username, json_data.data.password);
        // }
    }
}
